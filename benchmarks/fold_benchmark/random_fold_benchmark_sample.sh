...
if [ ! -f $DPATH/random_fold_sample_${LENGTH}_${STRUCT}.out ]
then
    python $SCRIPT -l $LENGTH -s $STRUCT -m sample > $DPATH/random_fold_sample_${LENGTH}_${STRUCT}.out;
fi
