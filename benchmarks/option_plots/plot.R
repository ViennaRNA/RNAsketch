...
spec = matrix(c(
    'file','f', 1, "character",
    'x','x', 1, "character",
    'xlabel','l', 1, "character",
    'y','y', 1, "character",
    'ylabel','p', 1, "character",
    'title','t', 1, "character"
), byrow=TRUE, ncol=4);
opt = getopt(spec);
...
opt$ylabel = expression(paste("mean ", delta, "E [kcal]"));
}
...
for (i in 1:number_structures) {
    ...
    print(colNameMFE)
    colNameDiff<- paste(opt$y, toString(i-1), sep="_")
...
pdf(paste(opt$y, ".", opt$file, ".pdf", sep=""))
